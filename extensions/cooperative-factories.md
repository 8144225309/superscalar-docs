...
The protocol's safety properties — unilateral exit, L-stock redistribution TX, inverted timelocks — protect members identically regardless of the organizational structure.
...
- **Old state recovery**: old leaf/factory states are **structurally superseded** ([[pseudo-spilman-leaves|PS]] chaining at the leaves, the [[decker-wattenhofer-invalidation|DW]] race at the interior), and the coordinator's L-stock is clawed back by the [[l-stock-redistribution|redistribution TX]] — redistributed to members, not burned. The [[shachain-revocation|Poon-Dryja penalty]] is the separate mechanism that punishes a revoked **inner-channel** commitment.
...
- The coordinator earns nothing from cheating. The redistribution TX redistributes the coordinator's L-stock to members, so cheating strictly loses more value than any old state could recover. Economic rationality reinforces the cryptographic guarantees.
...
| Capability | Status |
|-----------|--------|
| N-of-N factory construction | Working |
...
| L-stock redistribution TX | Working |
