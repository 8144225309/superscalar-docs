...
> **Summary**: SuperScalar's mechanisms aren't only described in these docs — they've been **run end-to-end on real signet**, and the factory's whole on-chain life is there for you to inspect. This page follows the **life of a pseudo-Spilman channel factory** — created, funded, used with real Lightning payments, checkpointed, laddered to fresh epochs, and cooperatively retired — and then the exception paths: what happens when a party won't cooperate, and what happens when one cheats. Every on-chain step has a real transaction ID you can look up; the Lightning payments in between settle off-chain, as Lightning does. It also records, honestly, the things that did *not* go to plan, because those are findings too.
...
The payments settle off-chain as Lightning HTLCs, so the factory's on-chain footprint is the funding plus its eventual close — but the traffic in between was real. This one UTXO is the shared home for all 127 clients' balances. (How this flagship's own funds were settled — and how that differs from the live close in Exhibit 6 — is noted under *What N=127 demonstrated*.)
...
| height | txid | what it is |
|---|---|---|
| 312776 | [`72c0790e…`](https://mempool.space/signet/tx/72c0790e2e58a9970fe8f8d4ded10800fb55f1bc04cdd93f22267e58f401f25b) | 127-client factory funding |
...
**What the chain proves, and what we attest — kept separate on purpose.** On mempool.space, anyone can verify *directly*: **one input** (the shared factory UTXO) → **128 outputs**, spent by a single **key-path** Schnorr signature, with **12,694,268 sats** out against a **5,732-sat fee**, exactly conserving the **12,700,000-sat** input — nothing created or destroyed. Two things a taproot **key-path** spend deliberately *cannot* reveal — it is byte-for-byte indistinguishable whether one key signed or a hundred did — we **attest** from the software and its logs, we do not claim the chain shows them:
...
This exhibit both demonstrates the mechanism and **validates the production signing path end-to-end** — and getting there caught a real bug worth recording, which is exactly what this exhibition is for. The single-process `--test-distrib` *demo* path had been rebuilding the distribution TX with placeholder demo keys, which cannot spend a strong-key funding output; the fix routes the exhibit through the **actual** transaction the creation ceremony co-signs (`dist_signed_tx`), the same one a real deployment broadcasts.
...
The poison above is one arm of the recourse; the other is the **watchtower**, and it holds **no private keys** — only pre-signed penalty transactions in its `wt.db`. So even if the LSP vanishes, *anyone* can run a standalone watchtower that catches a revoked-state broadcast and confirms the penalty. We drove this end-to-end on **real signet**: a client broadcast a revoked channel-commitment, and a secret-less watchtower — **22 channel-commitment watches hydrated from `wt.db`, holding no keys** — detected the breach and got the **pre-signed penalty confirmed on-chain**, clawing back the breached ~30k-sat `to_local` so the cheater keeps nothing but the mining fee.

| height | txid | what it is |
|---|---|---|
| 312904 | [`08fe413e…`](https://mempool.space/signet/tx/08fe413edc0fd650c5b1f53eef461e0918a6274783579d4ed8be61bf74db4c15) | client broadcasts a **revoked channel-commitment** (the cheat) |
| **312907** | [`a5c9d2b7…`](https://mempool.space/signet/tx/a5c9d2b755a1bf358020ac880cd6387c5904f433f3a4caa256572751a2b36ddd) | **secret-less watchtower penalty** — `wt.db`-only, no keys, confirmed on-chain |

The detection layer is described in [Detecting LSP Misbehavior](#deep-dives/lsp-misbehavior), and the full penalty matrix — factory, sub-factory, and channel-commitment — with its on-chain proofs is collected in the [Security Model](#deep-dives/security-model).
...
127 clients is the *design maximum*, and the cooperative close works there **live**: Exhibit 6 (`d1468287`) is the genuine ceremony — a 127-client factory funded, paid through with real HTLCs, and closed by **all 128 signers** in one on-chain transaction (1 input → 128 outputs, confirmed on signet and conserving to the satoshi; aggregate-signature and per-client-amount details attested as noted in Exhibit 6). That is the claim that matters, and it is proven.

The cooperative path is an **optimization** layered on top of the unilateral guarantee: like every cooperative step it needs the participants online at closing time — a coordination property of the efficient path, never a safety one — which is exactly why every client keeps a unilateral exit (Exhibits 7–9) regardless. (The Exhibit 1 flagship's own funds were settled at teardown via the operator's 128-key aggregate spend, `0ca6b929`; Exhibit 6 is a separate, fresh factory closed by the live ceremony.)
...
All runs used strong, per-run keys (never publicly-derivable weak keys), so every output is recoverable by its operator and none is left sweepable by outsiders. After the exhibition, the single test operator — who ran every daemon, per Exhibit 6's caveat — cooperatively closed the bridge channel and swept the residual factory-funding-root outputs back to one wallet for cleanup, using the same MuSig reconstruction that produced the design-max aggregate spend. In a real deployment each client holds its own keys and recovers independently; only because one operator held them all here does cleanup collapse to a single sweep.
