...
|---|---|---|---|
| 1 | Create, fund & **use** (real Lightning payments) | [`143471b5…`](https://mempool.space/signet/tx/143471b5d1ddc0eee3ea54d74ed17081f24d48f429bb826723c8b0897e55c0e6) | 312349 |
...
| 6 | **Retire** cooperatively at design-max (128-of-128 MuSig) | [`0ca6b929…`](https://mempool.space/signet/tx/0ca6b929e2d7a52633b33d3a0a36f531d6230f49ffccbac7486977d745aa1056) | 312535 |
...
**Part III — Adversarial: punishing a cheating LSP** (the headline security property)
...
The story it tells top-to-bottom: a factory is **created and used** → clients get **JIT inbound** → balances **checkpoint** → it **ladders** to fresh epochs → it **retires cooperatively**. Only then: *what if someone won't cooperate* (unilateral exit, timelocks, expiry distribution), and finally *what if someone cheats* (the poison).
...
- **Punish a cheat** — if the LSP broadcasts a superseded state to steal, the client's revealed-secret *poison* transaction redistributes the LSP's own stake to the clients.
...
The payments settle off-chain as Lightning HTLCs, so the factory's on-chain footprint is the funding plus its eventual close — but the traffic in between was real. This one UTXO is the shared home for all 127 clients' balances for the rest of its life below.
...
## 6. Retiring the design-max factory in one signature

To close the 127-client factory (Exhibit 1), all 128 participants agree to spend the funding output. On-chain, that agreement is **a single transaction with one Schnorr signature aggregating all 128 keys** — MuSig2 at the design maximum, on a real chain.
...
# Part III — Adversarial: punishing a cheating LSP

The happy path keeps funds safe when parties cooperate; the fallbacks keep them safe when parties simply vanish. This last exhibit is the one that keeps them safe when a party is actively **malicious** — the headline security property.
